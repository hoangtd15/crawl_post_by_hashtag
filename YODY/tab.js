...
function getPost() {
  ...
  for (var i = 0; i < posts.length; i++) {
    ...
    var linkss = links[0].href
    var link = ""
    var id = ""
    if(linkss.includes("vanity")){
      link= linkss
    
    }
    else if(linkss.includes("profile")){
      link = linkss.substring(0,linkss.indexOf("&"))
      id =  "&" + linkss.substring(link.indexOf("?")+1)
    } else{
      link = linkss.substring(0,linkss.indexOf("?"))
    }
    if (postShare.length < 1 && !link.toUpperCase().includes("YODY")) {
      ...
      if(photo != undefined){
        ...
        if(postId.includes("set=pcb.")){
          linkPost = (postId.substring(postId.indexOf("set=pcb.")+8,postId.indexOf("&__")))
        }else if(postId.includes("set=a.")){
          linkPost = (postId.substring(postId.indexOf("set=a.")+6,postId.indexOf("&__")))
        ...
        if(postId2.includes("set=pcb.")){
          linkPost = (postId2.substring(postId2.indexOf("set=pcb.")+8,postId2.indexOf("&__")))
        }else if(postId2.includes("set=a.")){
          linkPost = (postId2.substring(postId2.indexOf("set=a.")+6,postId2.indexOf("&__")))
        ...
        if(postId5.includes("set=pcb.")){
          linkPost = (postId5.substring(postId5.indexOf("set=pcb.")+8,postId5.indexOf("&__")))
        }else if(postId.includes("set=a.")){
          linkPost = (postId5.substring(postId5.indexOf("set=a.")+6,postId5.indexOf("&__")))
        }else if(postId5.includes("commerce")){
          linkPost = (postId5)
        }
      }
      var linkPosts = ""
      if(linkPost == undefined){
        linkPosts=""
      }
      else if(linkPost.includes("vanity") || linkPost.includes("commerce")){
        linkPosts = linkPost
      }else if(id != ""){
        linkPosts = "https://www.facebook.com/permalink.php?story_fbid=" + linkPost + id
      }else{
        linkPosts = link + "/posts/"+linkPost
      }
  }

    value.push(linkPosts)
